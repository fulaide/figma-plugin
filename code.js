            ...
            for (const collection of collections) {
                ...
                for (const mode of collection.modes) {
                    ...
                    for (const varName of varNames) {
                        const varDetails = mode.variables[varName];
                        // Try to find existing variable
                        let variable;
                        const existingVariables = figma.variables.getLocalVariables();
                        const existingVariable = existingVariables.find(v => v.name === varName && v.variableCollectionId === variableCollection.id);
                        if (existingVariable) {
                            variable = existingVariable;
                        }
                        else {
                            // Create a new variable
                            variable = figma.variables.createVariable(varName, variableCollection.id, convertType(varDetails.type));
                        }
